-- Fix timezone consistency: compute occurrences in IST (Asia/Kolkata) so the
-- server's missed-dose instants match what Indian elder devices write (which use
-- the device's local offset). This keeps the (source_id, due_at) dedup working
-- and the grace-window comparison correct near midnight.
create or replace function public.scan_missed_doses()
returns void language plpgsql security definer set search_path = public as $$
declare
  v_grace int;
  v_today date := (now() at time zone 'Asia/Kolkata')::date;
  v_bit   int  := (1 << (extract(isodow from (now() at time zone 'Asia/Kolkata'))::int - 1));
begin
  ...
  select s.elder_id, 'schedule', s.id, v_today,
         ((v_today::text || ' ' || s.time || ':00')::timestamp at time zone 'Asia/Kolkata'), 'missed'
  from medicine_schedules s
  join medicines m on m.id = s.medicine_id
  join elders e on e.id = s.elder_id
  where s.enabled and m.is_active and e.is_active
    and (s.days & v_bit) <> 0
    and now() > ((v_today::text || ' ' || s.time || ':00')::timestamp at time zone 'Asia/Kolkata') + (v_grace || ' minutes')::interval
  ...
  select l.elder_id, gl.guardian_id, 'missed_dose', l.id,
         'Missed medicine', e.name || ' missed a dose at ' || to_char(l.due_at at time zone 'Asia/Kolkata', 'HH24:MI')
  ...
  where l.occurrence_date = v_today and l.status = 'missed'
    and not exists (select 1 from alerts a where a.kind = 'missed_dose' and a.ref_id = l.id and a.guardian_id = gl.guardian_id);
end $$;
